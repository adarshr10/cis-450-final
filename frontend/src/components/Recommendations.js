...
export default class Recommendations extends React.Component {
	...
	submitMovie() {
		fetch(`http://localhost:8081/movierecs/${this.state.movieName}`, {method: 'GET'})
		.then(res => {
			return res.json();
		}, err => {
			...
			const recRows = recs.map(({title, movie_id, rating, num_ratings}) => 
				<RecommendationsRow
					title={title}
					movie_id={movie_id}
					rating={rating}
			...
			this.setState({recMovies: recRows});
		}, err => {
			console.error(err); // Print the error if there is one.
		});
	};
